<?php
	class Photoupload {
		...
		public $file_type; //alguses saadame siia parameetrina, hiljem teeb klass selle ise kindlaks
		...
		public function add_watermark($watermark_file){
			...
			echo "Lisan vesimärki!";
			imagedestroy($watermark);
		}
		
		function save_photo($target){
			if($this->file_type == "jpg"){
				if(imagejpeg($this->new_temp_photo, $target, 95) == false){
					$this->error = "Pildi salvestamine ebaõnnestus!";
				}
			}
